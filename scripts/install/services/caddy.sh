...
${RAILS_HOST} {
    ...
    # Use handle_path to strip the /gotify prefix before forwarding
    handle_path /gotify/* {
    ...
    # Redirect /gotify to /gotify/ for proper Gotify UI loading
    handle /gotify {
        redir /gotify/ permanent
    }
...
EOF
    ...
    # Add dedicated Gotify subdomain if configured
    if [ -n "${GOTIFY_HOST:-}" ] && [ "${GOTIFY_INSTALL_LOCAL:-}" == "true" ]; then
      cat >> "${caddyfile}" << EOF

# Gotify on dedicated subdomain (for mobile apps)
${GOTIFY_HOST} {
    tls ${ADMIN_EMAIL}
    encode gzip zstd
...
EOF
      info "Added Gotify subdomain: ${GOTIFY_HOST}"
    fi
  else
...
EOF
    ...
    # Add dedicated Gotify subdomain for HTTP mode if configured
    if [ -n "${GOTIFY_HOST:-}" ] && [ "${GOTIFY_INSTALL_LOCAL:-}" == "true" ]; then
      cat >> "${caddyfile}" << EOF

# Gotify on dedicated subdomain (HTTP mode)
${GOTIFY_HOST} {
    auto_https off
    encode gzip zstd
